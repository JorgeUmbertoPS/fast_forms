<?php

namespace App\Policies;

use Illuminate\Auth\Access\Response;
use App\Models\Modalidade;
use App\Models\User;

class ModalidadePolicy
{
    public function before($user, $ability){
        return $user->hasRole(['root','admin']);
    }
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->checkPermissionTo('view-any Modalidade');
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Modalidade $modalidade): bool
    {
        return $user->checkPermissionTo('view Modalidade');
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->checkPermissionTo('create Modalidade');
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Modalidade $modalidade): bool
    {
        return $user->checkPermissionTo('update Modalidade');
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Modalidade $modalidade): bool
    {
        return $user->checkPermissionTo('delete Modalidade');
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Modalidade $modalidade): bool
    {
        return $user->checkPermissionTo('restore Modalidade');
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Modalidade $modalidade): bool
    {
        return $user->checkPermissionTo('force-delete Modalidade');
    }
}
